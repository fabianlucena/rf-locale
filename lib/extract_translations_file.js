import {stripQuotes, levenshtein} from './utils.js';
            ...
            if (!currentTranslation) {
                ...
                if (!currentTranslation) {
                    ...
                    if (!currentTranslation) {
                        ...
                        if (!currentTranslation) {
                            let levenshteinLevel = 0,
                                levenshteinMax = Math.max(source.length / 5, 3);
                            
                            while (!currentTranslation && levenshteinLevel < levenshteinMax) {
                                currentTranslation = currentTranslations.find(translation =>
                                    translation
                                    && levenshtein(translation.source, source) < levenshteinLevel
                                    && translation.isPlural == data.isPlural
                                    && translation.isJson == data.isJson
                                );
                                levenshteinLevel++;
